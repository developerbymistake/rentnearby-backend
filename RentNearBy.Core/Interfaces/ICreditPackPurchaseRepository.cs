...
public interface ICreditPackPurchaseRepository
{
    Task AddAsync(CreditPackPurchase purchase);
    Task<CreditPackPurchase?> GetByIdAsync(Guid id);
    Task<CreditPackPurchase?> GetByRazorpayOrderIdAsync(string orderId);
    Task<IEnumerable<CreditPackPurchase>> GetByUserIdAsync(Guid userId);
    ...
    // order (e.g. re-requesting a credit pack after the app-level 20-min reuse window). A tracked-entity
