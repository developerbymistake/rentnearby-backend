...
public interface ICreditTransactionRepository
{
    ...
    Task<PagedResult<CreditTransaction>> GetPagedForUserAsync(Guid userId, int page, int pageSize, string? reason);
    ...
    Task<(IReadOnlyList<CreditTransactionWithUserResponse> Items, bool HasMore)> GetKeysetPagedWithUserAsync(
