...
{
    // Consumer "My Enquiries" — shared across both verticals in one list.
    Task<IEnumerable<Enquiry>> GetByUserIdAsync(Guid userId);

    // Server-anchored "My Enquiries" badge count — not-Closed AND updated since the consumer last saw
    ...
    Task<(IReadOnlyList<Enquiry> Items, bool HasMore)> GetAdminFilteredPagedAsync(
    ...
    Task<(IReadOnlyList<Enquiry> Items, bool HasMore)> GetByAssignedAgentIdAsync(
        Guid agentId, int page, int pageSize);

    // Lean membership check against the EnquiryAgent join table directly — for ownership gates that
    // run against a plain (tracked, navigation-not-loaded) GetByIdAsync fetch, where
    // enquiry.EnquiryAgents can't be relied on without a separate eager-load just for one boolean.
    Task<bool> IsAgentAssignedAsync(Guid enquiryId, Guid agentId);

    // "My Leads" badge count — not-Closed AND updated since this agent last saw it
    // (SeenAt == null || Enquiry.UpdatedAt > SeenAt). Replaces the old Status == "Submitted" filter,
    // which was structurally broken: both assignment code paths flip an enquiry to Contacted in the
    ...
    Task MarkSeenByUserAsync(Guid enquiryId, Guid userId);
    Task MarkSeenByAgentAsync(Guid enquiryId, Guid agentId);

    // For EnquiryDetailDto assembly: Service -> ServiceCategory, ServicePackage,
    // AssignedAgent -> Agent, StatusHistory -> ChangedByAdmin, all in one query.
    Task<Enquiry?> GetByIdWithDetailsAsync(Guid id);
