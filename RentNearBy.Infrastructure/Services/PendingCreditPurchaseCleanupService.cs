...
// Standalone sweep for CreditPackPurchase — does NOT extend PendingPaymentCleanupService (which swept
...
//     never landed (a crash between CreditPackPurchaseService.VerifyAndCreditAsync's two steps).
//   Pass B asks Razorpay directly (via ICreditPackPurchaseService.ReconcileWithRazorpayAsync) whether
...
public class PendingCreditPurchaseCleanupService : BackgroundService
{
    private readonly IServiceScopeFactory _serviceScopeFactory;
    private readonly ILogger<PendingCreditPurchaseCleanupService> _logger;
    ...
    public PendingCreditPurchaseCleanupService(IServiceScopeFactory serviceScopeFactory, ILogger<PendingCreditPurchaseCleanupService> logger)
    {
    ...
    {
        _logger.LogInformation("PendingCreditPurchaseCleanupService starting, interval={IntervalMinutes}min", Interval.TotalMinutes);
    ...
    private async Task RunSweepAsync()
    {
        try
        {
            ...
            var purchaseService = scope.ServiceProvider.GetRequiredService<ICreditPackPurchaseService>();
            var walletNotifier = scope.ServiceProvider.GetRequiredService<IWalletNotifier>();
            var cutoff = DateTime.UtcNow - AbandonedAfter;
            var now = DateTime.UtcNow;

            var stuckPending = await db.CreditPackPurchases
                .Where(p => p.Status == CreditPackPurchaseStatuses.Pending && p.CreatedAt < cutoff)
                .ToListAsync();

            // Pass A — self-heal a purchase whose wallet credit already succeeded locally but whose
            // own Status flip never landed (crash between CreditWalletService.AddCreditsAsync and
            // MarkSuccessIfPendingAsync in VerifyAndCreditAsync).
            var healedCount = 0;
            foreach (var purchase in stuckPending)
            {
                var alreadyCredited = await db.CreditTransactions.AnyAsync(t =>
                    t.Reason == CreditTransactionReasons.Recharge && t.ReferenceId == purchase.Id);
                if (alreadyCredited)
                {
                    purchase.Status = CreditPackPurchaseStatuses.Success;
            ...
            foreach (var purchase in stuckPending.Where(p => p.Status == CreditPackPurchaseStatuses.Pending))
            {
                try
                {
                    ...
                    {
                        case CreditPackReconcileOutcome.Credited:
                            reconciledCount++;
                            if (result.CreditResponse != null)
                            {
                                await walletNotifier.NotifyBalanceChangedAsync(
                                    purchase.UserId, result.CreditResponse.NewBalance, CreditTransactionReasons.Recharge);
                            }
                            break;
                        case CreditPackReconcileOutcome.NotPaid:
                            notPaidIds.Add(purchase.Id);
                            break;
                        case CreditPackReconcileOutcome.Inconclusive:
                            if (now - purchase.CreatedAt > InconclusiveGiveUpAfter)
                            {
                                staleInconclusiveIds.Add(purchase.Id);
                                _logger.LogError(
                                    "PendingCreditPurchaseCleanupService: purchase {PurchaseId} (order {OrderId}) still inconclusive with Razorpay after 24h — abandoning, needs manual review",
                ...
                catch (Exception ex)
                {
                    ...
                    reconcileErrorCount++;
                    _logger.LogError(ex, "PendingCreditPurchaseCleanupService: reconciliation failed for purchase {PurchaseId}", purchase.Id);
                }
            }
            ...
            var notPaidCount = notPaidIds.Count == 0 ? 0 : await db.CreditPackPurchases
                .Where(p => notPaidIds.Contains(p.Id) && p.Status == CreditPackPurchaseStatuses.Pending)
            ...
            var staleCount = staleInconclusiveIds.Count == 0 ? 0 : await db.CreditPackPurchases
                .Where(p => staleInconclusiveIds.Contains(p.Id) && p.Status == CreditPackPurchaseStatuses.Pending)
                .ExecuteUpdateAsync(s => s
                    .SetProperty(p => p.Status, CreditPackPurchaseStatuses.Abandoned)
            ...
            if (stuckPending.Count > 0)
                _logger.LogInformation(
                    "PendingCreditPurchaseCleanupService: stuckPending={StuckPending}, healed={Healed}, reconciled={Reconciled}, abandoned={Abandoned}, stillInconclusive={StillInconclusive}, reconcileErrors={ReconcileErrors}",
                    stuckPending.Count, healedCount, reconciledCount, abandonedCount, stillInconclusiveCount, reconcileErrorCount);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error in PendingCreditPurchaseCleanupService sweep");
        }
    }
}
